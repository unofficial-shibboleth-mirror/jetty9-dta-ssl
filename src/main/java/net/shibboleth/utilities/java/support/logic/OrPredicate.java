...
/**
 * A predicate that returns true if any composed predicate returns true. Note, this predicate does perform a
 * short-circuit evaluation so predicates occurring after the first predicate to return <code>true</code> will not be
 * evaluated.
 * 
 * @param <Input> type of argument
 */
public class OrPredicate<Input> implements Predicate<Input> {

    /** Predicates composed by a logical OR. */
    private final Predicate<? super Input>[] predicates;

    /**
     * Constructor.
     * 
     * @param composedPredicates predicates composed by a logical OR
     */
    public OrPredicate(@NotNull @NoNullElements final Predicate<? super Input>... composedPredicates) {
